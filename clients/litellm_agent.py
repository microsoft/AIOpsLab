...
"""LiteLLM client (with shell access) for AIOpsLab.

LiteLLM provides access to multiple AI models through a unified API with proper
exception handling for API errors. This addresses the AttributeError issue when
LiteLLM encounters API errors and tries to access exception.request on exceptions
that don't have this attribute.

More info: https://github.com/BerriAI/litellm
"""
...
class LiteLLMAgent:
    """Agent using LiteLLM with proper exception handling for API errors."""
    ...
    async def get_action(self, input) -> str:
        ...
        try:
            ...
            print(f"===== Agent (LiteLLM - {self.model}) ====\n{response[0]}")
            self.history.append({"role": "assistant", "content": response[0]})
            return response[0]
        except Exception as e:
            # Handle the AttributeError case and other exceptions gracefully
            # Use getattr to safely access exception attributes
            error_msg = str(e)
            request_info = getattr(e, 'request', None)
            response_info = getattr(e, 'response', None)
            
            print(f"LiteLLM API error: {error_msg}")
            if request_info:
            ...
            fallback_response = f"Error occurred while calling LiteLLM API: {error_msg}"
...
def setup_results_directory(model: str, agent_name: str = "litellm") -> Path:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run LiteLLM agent on AIOpsLab problems')
    ...
    parser.add_argument('--model', type=str,
                       default=os.getenv("LITELLM_MODEL", "openai/gpt-4o-mini"),
                       help='LiteLLM model to use (e.g., openai/gpt-4o-mini, openrouter/qwen/qwen3-8b)')
