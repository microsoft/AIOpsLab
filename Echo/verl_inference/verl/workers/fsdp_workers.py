...
class ActorRolloutRefWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_model_weights(self, model_state_dict: dict):
        """
        Load model weights from a state_dict into the actor module.
        """
        # only support load model weights for actor
        assert self._is_actor
        import torch
        ...
        try:
            ...
            fullstate_load_policy = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)

            with FSDP.state_dict_type(self.actor_module_fsdp, StateDictType.FULL_STATE_DICT, fullstate_load_policy):
                if torch.distributed.get_rank() == 0:
                    processed_state_dict = {}
                    for key, value in model_state_dict.items():
                        if isinstance(value, torch.Tensor):
                            processed_state_dict[key] = value.detach().cpu()
                        else:
                            processed_state_dict[key] = value
                    
                    print(f"[Worker] Loading {len(processed_state_dict)} model parameters")
                else:
                    processed_state_dict = {}
                
                missing_keys, unexpected_keys = self.actor_module_fsdp.load_state_dict(
                    processed_state_dict, strict=False
                )
                
                if torch.distributed.get_rank() == 0:
                    print(f"[Worker] Successfully loaded model weights")
                    if missing_keys:
                        print(f"[Worker] Missing keys: {missing_keys[:5]}...") 
                    if unexpected_keys:
                        print(f"[Worker] Unexpected keys: {unexpected_keys[:5]}...")  
            
            return True
            
        except Exception as e:
            if torch.distributed.get_rank() == 0:
                print(f"[Worker] Error loading model weights: {e}")
                import traceback
                traceback.print_exc()
            return False
        finally:
