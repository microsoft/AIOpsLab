...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
        ...
        elif rollout_name == "petals":
            from verl.workers.rollout.petals_rollout import PetalsRollout
            from verl.workers.sharding_manager.fsdp_petals import FSDPPetalsShardingManager

            log_gpu_memory_usage(f"Before building {rollout_name} rollout", logger=logger)
            local_path = copy_to_local(self.config.model.path)

            rollout = PetalsRollout(
                # model_path=local_path,
                config=self.config.rollout,
                module=self.actor_module_fsdp,
                tokenizer=self.tokenizer,
            ...
            # if torch.distributed.get_world_size() == 1:
            #     self.config.rollout.load_format = "dummy_hf"
            # rollout_sharding_manager = FSDPPetalsShardingManager(
            #     module=self.actor_module_fsdp,
            #     inference_engine=rollout.inference_engine,
            #     model_config=self.actor_model_config,
            #     full_params="hf" in self.config.rollout.load_format,
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            if self.config.rollout.name == "petals":
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_model_weights(self, local_path):
        """
        只保存actor模型的safetensors参数文件，用于模型同步
        与save_checkpoint并列的轻量级保存函数
        """
        # only support save model weights for actor
        assert self._is_actor
        import torch
        import os
        ...
        try:
            ...
            with FSDP.state_dict_type(self.actor_module_fsdp, StateDictType.FULL_STATE_DICT, fullstate_save_policy):
                model_state_dict = self.actor_module_fsdp.state_dict()
            
            # 只在rank 0上保存文件
            if torch.distributed.get_rank() == 0:
                # 过滤并处理状态字典，只保留tensor权重
                filtered_state = {}
                for key, value in model_state_dict.items():
                    if isinstance(value, torch.Tensor):
                        # 确保tensor在CPU上且是连续的，并且创建新的副本
                        tensor_cpu = value.detach().cpu().contiguous()
                        # 创建新的tensor副本以避免存储指针问题
                        filtered_state[key] = tensor_cpu.clone()
                
                if filtered_state:  # 确保有参数要保存
                    # 保存为safetensors格式
                    save_file(filtered_state, local_path)
                    
                    print(f"[Worker] Saved {len(filtered_state)} model parameters to {local_path}")
                    file_size = os.path.getsize(local_path)
                    print(f"[Worker] Model weights file size: {file_size} bytes")
                else:
                    print(f"[Worker] Warning: No valid parameters found to save")
            
        except Exception as e:
            if torch.distributed.get_rank() == 0:
                print(f"[Worker] Error saving model weights to {local_path}: {e}")
                import traceback
                traceback.print_exc()
            raise
        finally:
